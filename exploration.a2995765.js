...
exports.setSizing = setSizing;
exports.appendAxes = appendAxes;
exports.createXScale = createXScale;
exports.createYScale = createYScale;
...
exports.rotateXTicks = rotateXTicks;
...
function drawHeatmap(vizData, ligne, girouette) {
  d3.select("#heatmap-svg").selectAll().remove(); //document.getElementById('heatmap-svg').innerHTML = ''
...
function createXScale(flattenData) {
  ...
  var arrets = map_arret.keys();
  var arret_sort = arrets.sort(function (a, b) {
    return d3.ascending(+a, +b);
  });

  var nom_arret_sort = _toConsumableArray(arret_sort.map(function (a) {
    return map_arret.get(a);
  }));
...
function createColorScale(flattenData) {
  var retard = flattenData.map(function (d) {
    return d.moyenne;
  });
  var dom = d3.extent(retard);
  var color = d3.scaleLinear().domain([dom[0], 0, 0.01, dom[1]]).range([ROUGE, BLANC, VERT_NEUTRE, VERT]);
  return color;
} // ===================== DRAW RECTANGLES =====================
...
exports.generateBottomGraph = generateBottomGraph;
exports.generateCandleGraph = generateCandleGraph;
...
function getData(vizData, line, direction, trajectNumber) {
  var posLigne = vizData.findIndex(function (e) {
    return e.ligne === line;
  });
  var posGirouette = vizData[posLigne].girouettes.findIndex(function (e) {
...
function generateTopGraph(container, data) {
  // Generate common graph
  data.title = 'Variation de la ponctualité';

  var _generateCandleGraph = generateCandleGraph(container, data),
      _generateCandleGraph2 = _slicedToArray(_generateCandleGraph, 1),
      svg = _generateCandleGraph2[0];
}
/**
 * @param {Selection} container The div to generate the graph in
 * @param {object} data The data to fetch
...
function generateBottomGraph(container, data) {
  // Generate common graph
  data.title = 'Nombre de montants moyen';

  var _generateBarGraph = generateBarGraph(container, data),
      _generateBarGraph2 = _slicedToArray(_generateBarGraph, 1),
      svg = _generateBarGraph2[0];
}
/**
 * @param {Selection} container The div to generate the graph in
...
function generateCandleGraph(container, data) {
  // ===================== SETUP =====================
  // Delete existing content
  container.html(''); // Set size

  var width = container.node().getBoundingClientRect().width;
  var height = container.node().getBoundingClientRect().height; // Create svg
  ...
  var svg = container.append('svg').attr("width", width).attr("height", height); // ===================== OTHER =====================
  // X axis

  var xScale = d3.scaleBand().range([margin.left, width - margin.right]).domain(data.stops); // Add Y axis

  var yScale = d3.scaleLinear().domain([Math.min.apply(Math, _toConsumableArray(data.delay)), Math.max.apply(Math, _toConsumableArray(data.delay))]).range([height - margin.bottom, margin.top]);
  svg.append("g").attr("transform", "translate(" + margin.left + ", 0 )").call(d3.axisLeft(yScale)); //Y Title

  svg.append("text").attr("transform", "translate(" + (margin.right - 80) + "," + (margin.top - 10) + ")").append('tspan').attr('x', 0).attr('dy', 5).text('Minutes').attr('fill', '#898989').attr('font-size', 12); // Draw axis line 0 

  var xAxis = svg.append('g').attr('id', 'x-axis'); // Draw axis line

  xAxis.append('path').attr('d', d3.line()([[margin.left, yScale(0)], [width - margin.left, yScale(0)]])).attr('stroke', 'black').style("stroke-dasharray", "3,3"); // Draw axis line 5 
...
function generateBarGraph(container, data) {
  ...
  var xScale = d3.scaleBand().range([margin.left, width - margin.right]).domain(data.stops);
  svg.append("g").attr("transform", "translate(0," + (height - margin.bottom) + ")").call(d3.axisBottom(xScale)).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").attr("class", function (d, i) {
    return "axisText" + i;
  }).style("text-anchor", "end"); //X Title

  svg.append("text").attr("transform", "translate(" + (width - margin.right) + "," + (height - margin.bottom + 25) + ")").text('Arrêt').attr('fill', '#898989').attr('font-size', 12); // Add Y axis

  var yScale = d3.scaleLinear().domain([0, Math.max.apply(Math, _toConsumableArray(data.amounts))]).range([height - margin.bottom, margin.top]);
  svg.append("g").attr("transform", "translate(" + margin.left + ", 0 )").call(d3.axisLeft(yScale)); //Y Title

  svg.append("text").attr("transform", "translate(" + (margin.right - 80) + "," + (margin.top - 10) + ")").append('tspan').attr('x', 0).attr('dy', 5).text('Nombre').attr('fill', '#898989').attr('font-size', 12).append('tspan').attr('x', 0).attr('dy', 11).text('de').append('tspan').attr('x', 0).attr('dy', 10).text('personnes'); // ===================== HOVER =====================
  // Bars
...
},{"@popperjs/core":"../node_modules/@popperjs/core/lib/index.js"}],"exploration.js":[function(require,module,exports) {
'use strict';
...
var candlestick = _interopRequireWildcard(require("./scripts/candlestick"));
...
},{"./scripts/helper.js":"scripts/helper.js","./scripts/heatmap.js":"scripts/heatmap.js","./scripts/preprocess.js":"scripts/preprocess.js","./scripts/candlestick":"scripts/candlestick.js","bootstrap":"../node_modules/bootstrap/dist/js/bootstrap.esm.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","exploration.js"], null)
//# sourceMappingURL=/exploration.a2995765.js.map